...
<template>
  <div class="all">
    <el-form :inline="true" class="demo-form-inline">
      ...
      <el-form-item>
        <el-button type="success" @click="exportExcel"  style="font-size: 20px;"> 导出Excel</el-button>
      </el-form-item>
      <el-form-item>
        <el-button type="warning" @click="exportPDF"  style="font-size: 20px;"> 导出PDF</el-button>
      </el-form-item>
      <el-form-item>
        <el-button type="info" @click="printData"  style="font-size: 20px;"> 打印</el-button>
      </el-form-item>
    </el-form>
...
export default {
  ...
  methods: {
    ...
    exportExcel() {
      ...
      // 请求全部数据
      this.$axios(`/api/students/1/9999/${name}/${grade}/${tel}/${institute}/${major}/${clazz}`).then(res => {
        if (res.data.code == 200) {
          const data = res.data.data.records;
          const columns = [
            { prop: 'studentId', label: '学号' },
            { prop: 'studentName', label: '姓名' },
            { prop: 'institute', label: '学院' },
            { prop: 'major', label: '专业' },
            { prop: 'grade', label: '年级' },
            { prop: 'clazz', label: '班级' },
            { prop: 'sex', label: '性别' },
          ...
          exportToExcel(data, columns, '学生成绩列表');
    ...
    // 导出PDF（调用后端API）
    exportPDF() {
      // 直接访问后端API导出所有学生成绩PDF
      window.open('http://localhost:9201/scores/pdf', '_blank');
