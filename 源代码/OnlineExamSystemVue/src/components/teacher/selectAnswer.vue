...
export default {
  ...
  methods: {
    ...
    exportExcel() {
      // 请求全部数据
      var subject = this.subject.trim() == "" ? "@" : this.subject;
      var section = this.section.trim() == "" ? "@" : this.section;
      var question = this.question.trim() == "" ? "@" : this.question;

      this.$axios(`/api/answers/1/9999/${subject}/${section}/${question}`).then(res => {
        if (res.data.code == 200) {
          const data = res.data.data.records;
          const columns = [
            { prop: 'subject', label: '试卷名称' },
            { prop: 'question', label: '题目信息' },
            { prop: 'section', label: '所属章节' },
            { prop: 'type', label: '题目类型' },
            { prop: 'score', label: '试题分数' },
            { prop: 'level', label: '难度等级' }
          ];
          exportToExcel(data, columns, '题库列表');
