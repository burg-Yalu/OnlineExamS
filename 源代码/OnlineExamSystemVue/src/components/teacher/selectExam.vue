    ...
    exportExcel() {
      ...
      this.$axios('/api/exams').then(res => {
        if (res.data.code == 200) {
          ...
          const columns = [
            { prop: 'source', label: '试卷名称' },
            { prop: 'description', label: '介绍' },
            { prop: 'institute', label: '所属学院' },
            { prop: 'major', label: '所属专业' },
            { prop: 'grade', label: '年级' },
            { prop: 'examDate', label: '考试日期' },
            { prop: 'totalTime', label: '持续时间' },
            { prop: 'totalScore', label: '总分' },
            { prop: 'type', label: '试卷类型' },
            { prop: 'tips', label: '考生提示' }
          ];
          exportToExcel(data, columns, '考试列表');
